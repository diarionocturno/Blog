...
class SmartYouTube_PRO {
	...
	function post_header() {
			...
			if ( isset( $matches[0][5] ) )
				if ( $matches[0][5] != '' )
					echo '<meta property="og:image" content="http://i.ytimg.com/vi/' . $matches[0][5] . '/0.jpg" />';
	...
	function options_page() { 
		...
		if ( isset( $_POST['submitted'] ) ) {			
			...
			$this->options['widthside'] = ! isset( $_POST['disp_widthside'] ) ? 300 : intval( $_POST['disp_widthside'] );
			$this->options['heightside'] = ! isset( $_POST['disp_heightside'] ) ? 250 : intval( $_POST['disp_heightside'] );
...
EOT;
		...
		if ( is_feed() && ( $context == 'post' || $context = 'excerpt' ) ) {
			...
			if ( $this->options['img'] == 'on' ) {
				$yte_tag .= '<p><img src="http://img.youtube.com/vi/' . $file . '/0.jpg" width="480" height="370" border=0></p>';
			}

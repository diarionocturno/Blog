...
if (isset($_GET['archives']) && ($archives = $_GET['archives'])!='') {
  ...
  if (is_search()) {
    ...
    $wpmp_title = sprintf(__("Archivo de '%s' tag", 'wpmp'), single_tag_title('', false));
  } elseif (is_category()) {
    $wpmp_title = sprintf(__("Archivo de '%s'", 'wpmp'), single_cat_title('', false));
  } elseif (is_day()) {
    $wpmp_title = sprintf(__("Archivo de %s", 'wpmp'), get_the_time('F jS, Y'));
  } elseif (is_month()) {
    $wpmp_title = sprintf(__("Archivo de %s", 'wpmp'), get_the_time('F, Y'));
  } elseif (is_year()) {
    $wpmp_title = sprintf(__("Archivo de %s", 'wpmp'), get_the_time('Y'));
  } elseif (is_author()) {
    $wpmp_title = __("Autor", 'wpmp');
  } elseif (isset($_GET['paged']) && !empty($_GET['paged'])) {
    $wpmp_title = __("DiarioNocturno.com", 'wpmp');
...
if ($wpmp_not_found) {
  ...
  if (file_exists($wpmp_include = wpmp_theme_group_file('index.php'))) {
    ...
    if(!is_single() && !is_page()) {
      ...
      next_posts_link(__('Anterior', 'wpmp'));
      print ' ';
      previous_posts_link(__('Siguiente', 'wpmp'));
      print '</p>';
    }
...
function wpmp_theme_post_single() {
  ...
  print '<p class="metadata">'; previous_post_link(__('Anterior:', 'wpmp') . ' %link'); print '<br />'; next_post_link(__('Siguiente:', 'wpmp') . ' %link'); print '</p>';
...
function wpmp_theme_post($single = false) {
  ...
  if ($single || ($summary!='none' && ($summary!='firstteaser' || $wpmp_summary_first))) {
    ...
    the_content(__('Leer mas', 'wpmp'));
    print '</div>';
    $wpmp_summary_first = false;
  }
  if ($single || $metadata) {
    print '<p class="metadata">' . __('Publicado en ', 'wpmp');
    the_category(', ');
    edit_post_link('Edit', ' | ', '');
    print ' | ';

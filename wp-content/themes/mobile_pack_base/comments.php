...
<?php
  if (file_exists($wpmp_include = wpmp_theme_group_file('comments.php'))) {
    ...
    if ($comments) {
      print '<h3 id="comments">'; comments_number('Sin comentarios', '1 comentario', '% comments' ); _e(' on this post.', 'wpmp') . '</h3>';
      wpmp_theme_comment_list($comments);
    }
    if ($post->comment_status == 'open') {
      print '<h3 id="respond">' . __('Comentarios', 'wpmp') . '</h3>';
  ...
  function wpmp_theme_comment_form($user_ID, $user_identity, $req, $comment_author, $comment_author_url, $id, $post) {
    ...
    <?php } else { ?>
      <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
        ...
        <?php } else { ?>
          <p>
            <label for="author"><?php _e('Nombre', 'wpmp'); ?> <?php if ($req) {_e("(requerido)", 'wpmp');} ?></label>
            <br />
            <input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" />
          </p>
          <p>
            <label for="email"><?php _e('Mail', 'wpmp'); ?> (<?php if ($req) {_e("Requerido, pero ", 'wpmp');} ?><?php _e("no publicado", 'wpmp'); ?>)</label>
            <br />
        ...
        <?php } ?>
        <p>
          <label for="comment"><?php _e('Comentario', 'wpmp'); ?></label>
          <br />
